...
/**
 * API agentic loop — thin entry point.
 *
 * All behavior lives in the generic loop (shared/agentic-loop.js) driven by the
 * "api" node in contracts.config.json. This file only resolves that node and
...
/**
 * Starts the API node's agentic loop.
...
if (require.main === module) {
  ...
  const handle = start();
  console.log('API agentic loop is running. Watching for contract changes...');
